...
{/* <script src="../assets/cat_config/catalog_operations.js"></script> */}
/**
...
/* ==========================================================================
   ...
   ========================================================================== */
const WEB_APP_URL = "https://script.google.com/macros/s/AKfycbwWt9pPH3i8NkTxNVGfgkHt_f4ZJJ5s7Uzxnjm9s0BBb6e_9hm17vAR_2LP_jwERq-cng/exec";
...
function abrirModalMaestroReceta() {
    const m = document.getElementById("formModalReceta");
    document.getElementById("formMaestroReceta")?.reset();
    document.getElementById("ingredlista-ientes-body").innerHTML = "";
    document.getElementById("section-relacionReceta").classList.add("hidden");
    document.getElementById("container-btn-relacionReceta").style.display = "none";
    document.getElementById("btnFinalizar").disabled = true;
    
    window.relacionRecetaTemporales= [];
    
    poblarSelectMaestroMaestroReceta("id_receta_bp", "CAT_PRODUCTOS", "ID_BP", "NOMBRE");
    poblarSelectMaestroMaestroReceta("id_sabor", "LST_SABORES", "ID_SABOR", "NOMBRE");
    poblarSelectMaestroMaestroReceta("id_categoria", "LST_CATEGORIAS", "ID_CATEGORIA", "NOMBRE");
...
function validarCamposRecetaBase() {
    ...
    document.getElementById("container-btn-relacionReceta").style.display =
        (n && c && s) ? "block" : "none";
}
...
function validarBaseReceta() {
    ...
    if (n && n.length > 2 && c && s) {
        document.getElementById("bloque-relacionReceta").style.display = "block";
        recetaoBaseTemp = { nombre: n, categoria: c, sabor: s };
        showToast("✅ Base validada. Agregue Relacion de Ingredientes.", "success");
    } else {
...
function activarSeccionRelacionIngredientes() {
    document.getElementById("section-relacionReceta").classList.remove("hidden");
    poblarSelectMaestroMaestroReceta("id_presentacion", "LST_PRESENTACIONES", "ID_PRESENTACION", "NOMBRE");
    poblarSelectMaestroMaestroReceta("id_mp", "CAT_MATERIA_PRIMA", "ID_MP", "NOMBRE");
    poblarSelectMaestroMaestroReceta("id_unidad", "LST_UNIDADES", "ID_UNIDAD", "ABREVIATURA");
}


function agregarRelacionIngredientesALista() {
    const p = document.getElementById("id_presentacion");
    const mp = document.getElementById("id_mp");
    const um = document.getElementById("id_unidad");
    const c = document.getElementById("cantidad")?.value;
    const ins = document.getElementById("instrucciones")?.value;
    const ccal = document.getElementById("costo_calculado")?.value;
    ...
    window.relacionRecetaTemporales.push({
        ID_MP: p.value || "",
        NOMBRE: p.options[p.selectedIndex]?.text || "",
        ID_UNIDAD: p.value || "",
        ABREVIATURA: p.options[p.selectedIndex]?.text || "",
        CANTIDAD: v || "",
        INSTRUCCIONES: v || "",
        COSTO_CALCULADO: v || "",
    });

    agregarRelacionIngredientesAListaRender();
...
function agregarRelacionIngredientesAListaRender() {
    const tbody = document.getElementById("ingredlista-ientes-body");
    tbody.innerHTML = window.relacionRecetaTemporales.map((item, i) => `
        <tr>
            <td>${item.NOMBRE_PRESENTACION || item.ID_PRESENTACION}</td>
            <td>${item.NOMBRE || item.ID_MP}</td>
            <td>${item.ABREVIATURA || item.ID_UNIDAD}</td>
            <td>${item.CANTIDAD} días</td>
            <td>${item.INSTRUCCIONES} días</td>
            <td>${item.COSTO_CALCULADO} días</td>
            <td>
    ...
    document.getElementById("btnFinalizar").disabled = window.relacionRecetaTemporales.length === 0;
}
...
function abrirModalMaestroProductos() {
    ...
    poblarSelectMaestroProductos("id_categoria", "LST_CATEGORIAS", "ID_CATEGORIA", "NOMBRE");
    poblarSelectMaestroProductos("id_sabor", "LST_SABORES", "ID_SABOR", "NOMBRE");
...
function activarSeccionVariantes() {
    ...
    poblarSelectMaestroProductos("id_presentacion", "LST_PRESENTACIONES", "ID_PRESENTACION", "NOMBRE");
}
...
function poblarSelectMaestroProductos(domId, source, key, display) {
